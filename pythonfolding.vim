...
function! GetPythonFold()
  ...
  let currentIndent = IndentLevel(v:lnum)
	...
	" Create new fold if we have a new function def, without preceding @patch decorators
	if currentLine =~# '\v^ *def ' && w:inPatch == 0
		if currentLine =~ '\v\([^\)]*$'
	...
	if currentLine =~# '\v^ *(class|def)'
		" We are no longer in a set of @patch decorators
		let w:inPatch = 0
	endif

	" Create new fold if we are starting a set of @patch decorators
	" Useful for folding unit test files
	if currentLine =~# '\v^ *\@patch' && w:inPatch == 0
		let w:inPatch = 1
		return StackIndent(currentIndent)
	endif
	...
	if w:currentLevel > 0 && currentIndent > 0 
		if w:inParens == 0
